# Adapted from transformers v4.57.3 for Mixtral MoE with rescale support
import math
...
from dysco.custom_mixin import CustomGenerationMixin
...
class RescaleMixtralAttention(nn.Module):
...
class RescaleMixtralDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: MixtralConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = RescaleMixtralAttention(config, layer_idx)

        # All Mixtral layers are MoE (no mlp_only_layers concept).
        # Module name `block_sparse_moe` is fixed by the pretrained checkpoint.
...
class MixtralPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["RescaleMixtralDecoderLayer"]
    ...
    _can_record_outputs = {
        ...
        "hidden_states": RescaleMixtralDecoderLayer,
        "attentions": RescaleMixtralAttention,
    }


@auto_docstring
class RescaleMixtralModel(MixtralPreTrainedModel):
    def __init__(self, config: MixtralConfig):
        ...
        self.layers = nn.ModuleList(
            [RescaleMixtralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
@auto_docstring
class RescaleMixtralForCausalLM(MixtralPreTrainedModel, CustomGenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = RescaleMixtralModel(config)
        self.vocab_size = config.vocab_size
